  ...
  const loadStats = async () => {
    ...
    if (membership) {
      const [people, sessions, checkins] = await Promise.all([
        supabase.from("people").select("id", { count: "exact", head: true }).eq("org_id", (membership as any).org_id),
        supabase.from("sessions").select("id", { count: "exact", head: true }).eq("org_id", (membership as any).org_id),
        supabase.from("attendance_records").select("id", { count: "exact", head: true }).eq("org_id", (membership as any).org_id),
      ]);
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="page-header">
        ...
        <p className="page-subtitle">
          View and export attendance analytics and reports.
        </p>
      </div>
