...
export default function OrgUsersPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const orgIdValue = (org as { id: string }).id;
      setOrgId(orgIdValue);
  ...
  const handleInvite = async () => {
    ...
    // TODO: Wire this up to real invite flow
    toast({
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          {
            label: "Owners",
            value: members.filter((m) => m.role === "org_owner").length,
            icon: Crown,
            color: "text-warning",
          },
          {
          ...
          {
            label: "Members",
            value: members.filter((m) => !["org_owner", "admin"].includes(m.role)).length,
            icon: User,
            color: "text-success",
          },
        ].map((stat, idx) => (
